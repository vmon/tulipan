...
I borrowed this file from a File Digest Program but I'm not interested in file
digesting. What I need is just a pure SHA implementation to use it for random
coefficient generation to obtain a ranodm polynomial.

So I have to get rid of any file I/O feature of it.
...
class C_L_SHA : public IMessageDigest
{
public:
	enum { SHA160=0, SHA256=1, SHA384=2, SHA512=3 };
	//CONSTRUCTOR
	C_L_SHA(int iMethod=SHA160);
...
inline unsigned int C_L_SHA::CircularShift(unsigned int uiBits, unsigned int uiWord)
{
	return (uiWord << uiBits) | (uiWord >> (32-uiBits));
}

inline unsigned int C_L_SHA::CH(unsigned int x, unsigned int y, unsigned int z)
{
	return ((x&(y^z))^z);
}

inline unsigned int C_L_SHA::MAJ(unsigned int x, unsigned int y, unsigned int z)
{
	return (((x|y)&z)|(x&y));
}

inline unsigned int C_L_SHA::SIG0(unsigned int x)
{
	return ((x >> 2)|(x << 30)) ^ ((x >> 13)|(x << 19)) ^ ((x >> 22)|(x << 10));
}

inline unsigned int C_L_SHA::SIG1(unsigned int x)
{
...
inline void C_L_SHA::Bytes2Word(unsigned char const* pcBytes, unsigned int& ruiWord)
{
	ruiWord = (unsigned int)*(pcBytes+3) | (unsigned int)(*(pcBytes+2)<<8) |
		(unsigned int)(*(pcBytes+1)<<16) | (unsigned int)(*pcBytes<<24);
}

inline void C_L_SHA::Word2Bytes(unsigned int const& ruiWord, unsigned char* pcBytes)
{
	pcBytes += 3;
...
inline SUI64 C_L_SHA::CircularShift(unsigned int uiBits, SUI64 const& roui64Word)
{
	return (roui64Word << uiBits) | (roui64Word >> (64-uiBits));
}

inline SUI64 C_L_SHA::CH(SUI64 const& x, SUI64 const& y, SUI64 const& z)
{
	return ((x&(y^z))^z);
}

inline SUI64 C_L_SHA::MAJ(SUI64 const& x, SUI64 const& y, SUI64 const& z)
{
	return (((x|y)&z)|(x&y));
}

inline SUI64 C_L_SHA::SIG0(SUI64 const& x)
{
	return ((x >> 28)|(x << 36)) ^ ((x >> 34)|(x << 30)) ^ ((x >> 39)|(x << 25));
}

inline SUI64 C_L_SHA::SIG1(SUI64 const& x)
{
...
inline void C_L_SHA::Bytes2Word(unsigned char const* pcBytes, SUI64& ruiWord)
{
	Bytes2Word(pcBytes+4, ruiWord.m_uiRight);
	Bytes2Word(pcBytes, ruiWord.m_uiLeft);
}

inline void C_L_SHA::Word2Bytes(SUI64 const& ruiWord, unsigned char* pcBytes)
{
